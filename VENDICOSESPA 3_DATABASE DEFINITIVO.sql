...
-- METODO 1
INSERT INTO sales (sales_order_number, sales_date, product_id, quantity_sold, unit_price, sales_amount, store_id) VALUES
('FT021', '2024-06-25', 42, 4, 7.00, 28.00, 15);
UPDATE sales SET Quantity_sold = 50 WHERE id = 1403;

start transaction;
update inventory set quantity = quantity - (select quantity_sold from sales where id = 1403 and sales.product_id = inventory.product_id)
where product_id in (select product_id from sales where id = 1403) 
and warehouse_id = ( select warehousestores.warehouse_id from warehousestores join stores on warehousestores.store_id = stores.id
join sales on stores.id = sales.store_id where sales.id = 1403) limit 1;
...
-- METODO 3 - TRIGGER RIFERITO AL METODO 1
...
CREATE TRIGGER update_inventory_after_sale    -- Definiamo un trigger con un nome. Questo trigger si attiva dopo una insert sulla tabella Sales.
...
BEGIN                                          -- Inizio del blocco di istruzioni del trigger

    -- Aggiornamento della tabella Inventory, decrementando la quantità disponibile in magazzino
    UPDATE inventory                           -- Update utilizzato per diminuire la quantità disponibile nel magazzino
    SET quantity = quantity - NEW.quantity_sold -- NEW.quantity_sold si riferisce alla quantità venduta appena inserita nella tabella Sales per la riga attivata dal trigger
    WHERE product_id = NEW.product_id         -- NEW.product_id si riferisce al prodotto della riga appena inserita nella tabella Sales, identificando il prodotto da aggiornare nell'inventario
    AND warehouse_id = (                      -- Condizione che associa l'aggiornamento dell'inventario al magazzino giusto, selezionando il warehouse_id corretto
        SELECT warehousestores.warehouse_id   -- Subquery per ottenere l'ID del magazzino associato allo store della vendita
        FROM warehousestores                   -- Tabella che collega gli store ai magazzini
        JOIN stores ON warehousestores.store_id = stores.id  -- Join tra warehousestores e stores per ottenere l'ID dello store
        WHERE stores.id = NEW.store_id         -- NEW.store_id identifica lo store della riga appena inserita nella tabella Sales, assicurandosi che venga aggiornato il magazzino giusto
    );

END;                                          -- Fine del blocco di istruzioni del trigger
$$                                        
DELIMITER ;                                   -- Ripristina il delimitatore di fine comando a ;, il valore predefinito

-- METODO 4 - TRIGGER RIFERITO AL METODO 2
DELIMITER $$;                                  -- Serve a modificare temporaneamente il simbolo che indica il termine di un comando, così che SQL non interpreti ; come la fine del comando ma usi $$
CREATE TRIGGER update_inventory_after_sale    -- Definiamo un trigger con un nome. Questo trigger si attiva dopo una insert sulla tabella Sales.
AFTER INSERT ON sales                         -- Il trigger si attiverà ogni volta che viene effettuata una insert nella tabella Sales
FOR EACH ROW                                  -- Il trigger verrà eseguito per ogni riga inserita nella tabella Sales
BEGIN                                          -- Inizio del blocco di istruzioni del trigger
    -- Aggiornamento della tabella Inventory, decrementando la quantità disponibile in magazzino
    UPDATE inventory i                         -- Update utilizzato per diminuire la quantità disponibile nel magazzino. La tabella Inventory ha l'alias "i" per semplificare la scrittura.
    JOIN warehouses w ON i.warehouse_id = w.id -- Join tra la tabella Inventory e la tabella Warehouses per collegare il magazzino giusto usando l'ID del magazzino.
    JOIN warehousestores ws ON w.id = ws.warehouse_id  -- Join tra Warehouses e Warehousestores per trovare il magazzino giusto
    JOIN stores s ON ws.store_id = s.id        -- Join tra Warehousestores e Stores per identificare lo store corretto della vendita
    SET i.quantity = i.quantity - NEW.quantity_sold -- NEW.quantity_sold si riferisce alla quantità venduta appena inserita nella tabella Sales, e viene sottratta dalla quantità disponibile in magazzino
    WHERE i.product_id = NEW.product_id        -- NEW.product_id si riferisce al prodotto della riga appena inserita nella tabella Sales, identificando il prodotto da aggiornare nell'inventario
    AND s.id = NEW.store_id                    -- NEW.store_id identifica lo store della riga appena inserita nella tabella Sales, assicurandosi che venga aggiornato il magazzino giusto
...
-- 2° QUERY 
-- Quali sono le query da eseguire per verificare quante unità di un prodotto ci sono in un dato magazzino?
-- METODO 1 -- Vista per product_id in tutti i magazzini che evidenzia la quantità in inventario
create view Check_prodotti_magazzino as (
select inventory.warehouse_id, inventory.product_id, products.name, sum(inventory.quantity)
from inventory
join products on inventory.product_id = products.id
group by inventory.product_id, products.name, inventory.warehouse_id
);
Select * from Check_prodotti_magazzino;

-- METODO 2 -- Mostra il singolo prodotto per singolo magazzino
...
-- METODO 3 -- Mostra più prodotti selezionati per più magazzini selezionati
SELECT i.warehouse_id, p.name AS product_name, SUM(i.quantity) AS total_quantity
FROM inventory i
JOIN products p ON i.product_id = p.id
...
-- 3° QUERY
-- Quali sono le query da eseguire per monitorare le soglie di restock?
-- METODO 1
